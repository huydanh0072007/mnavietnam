...
export const DataRoomUnlock = ({ projectTitle, dict, lang }: { projectTitle: string, dict?: any, lang?: string }) => {
  ...
  const handleSignNDA = async () => {
    ...
    try {
      ...
      alert('Không thể mã hóa chữ ký. Vui lòng thử lại.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <AnimatePresence mode="wait">
        {step === 'locked' && (
          ...
          >
            ...
            <h3 className="text-xl font-serif font-bold text-[#C4A35A]">Virtual Data Room</h3>
            <p className="text-sm text-gray-300">
              Đây là dự án bảo mật. Vui lòng định danh và ký Thỏa thuận Bảo mật (NDA) điện tử để truy cập Hồ sơ thông tin chi tiết (IM), Pháp lý, và Mô hình tài chính.
            </p>
        ...
        {step === 'identity' && (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-lg font-serif font-bold text-[#C4A35A]">Định danh Nhà đầu tư</h3>
            </div>
            
            <div className="space-y-3">
              <input required type="text" placeholder="Họ và Tên" className="w-full bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm text-white focus:outline-none focus:border-[#C4A35A]" 
                value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
              <input required type="text" placeholder="Đơn vị công tác" className="w-full bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm text-white focus:outline-none focus:border-[#C4A35A]" 
                value={formData.org} onChange={e => setFormData({...formData, org: e.target.value})} />
              <input required type="email" placeholder="Email doanh nghiệp" className="w-full bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm text-white focus:outline-none focus:border-[#C4A35A]" 
                value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
              <input required type="text" placeholder="Số CCCD / Hộ chiếu" className="w-full bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm text-white focus:outline-none focus:border-[#C4A35A]" 
            ...
            <label className="flex items-start gap-2 mt-2 cursor-pointer group">
              ...
              <span className="text-xs text-gray-400 group-hover:text-gray-300 transition-colors leading-relaxed">
                Tôi đồng ý với Chính sách Bảo vệ Dữ liệu Cá nhân theo Luật số 91/2025/QH15. Dữ liệu của tôi chỉ được sử dụng cho mục đích đánh giá cơ hội đầu tư và tôi có quyền yêu cầu xóa bất kỳ lúc nào.
              </span>
            </label>
        ...
        {step === 'otp' && (
          ...
          >
            ...
            <h3 className="text-xl font-serif font-bold text-[#C4A35A]">Xác thực Email</h3>
            <p className="text-sm text-gray-300">
              Vui lòng nhập mã OTP 6 số vừa được gửi đến <strong>{formData.email || 'email của bạn'}</strong>.
            </p>
        ...
        {step === 'signing' && (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-lg font-serif font-bold text-[#C4A35A]">Ký NDA Điện tử</h3>
            </div>
            <div className="bg-white/5 p-4 rounded-lg text-xs text-gray-300 h-32 overflow-y-auto custom-scrollbar border border-white/10">
              <p className="mb-2 text-center text-sm text-white"><strong>THỎA THUẬN BẢO MẬT BÊN NHẬN THÔNG TIN</strong></p>
              <p className="mb-2"><strong>Dự án:</strong> {projectTitle}</p>
              <p className="mb-2"><strong>Đại diện:</strong> Ông/Bà {formData.name || '[Tên NĐT]'}, CCCD: {formData.cccd || '[CCCD]'}</p>
              <p>Bằng việc ký tên dưới đây, Bên Nhận cam kết không tiết lộ, sao chép hoặc chia sẻ bất kỳ tài liệu nào từ Virtual Data Room cho bên thứ ba. Mọi dữ liệu tải xuống sẽ được lưu vết (Audit Trail) và đóng dấu mờ (Watermark) mang định danh của Bên Nhận.</p>
              <p className="mt-2 text-gray-400">IP truy cập: {typeof window !== 'undefined' ? '12.34.56.78' : '...'}</p>
            </div>
            
            {/* Signature Canvas */}
            <div>
              <div className="flex justify-between items-center mb-1">
                <span className="text-xs text-gray-400">Vui lòng ký tên vào ô bên dưới:</span>
                <button onClick={clearSignature} className="text-xs text-[#C4A35A] hover:underline">Xóa chữ ký</button>
              </div>
              <canvas
                ref={canvasRef}
                width={300}
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button onClick={handleSignNDA} className="flex-1 shadow-lg shadow-[#C4A35A]/20" disabled={isLoading}>
                {isLoading ? 'Đang mã hóa...' : 'Hoàn tất Ký'}
              </Button>
            </div>
          </motion.div>
        )}
        ...
        {step === 'unlocked' && (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-lg font-serif font-bold text-green-400">Đã mở khóa Data Room</h3>
            </div>
            
            <div className="bg-green-500/10 border border-green-500/20 p-3 rounded-lg text-xs text-green-200 mb-2">
              Tài liệu hiển thị dưới đây đã được mã hóa tự động và đóng dấu mờ (watermark) theo thông tin định danh của bạn.
            </div>
